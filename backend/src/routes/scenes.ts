...
import { getDramaIdByEpisodeId, getDramaIdBySceneId, requireExistingDramaAccess } from '../middleware/auth.js'
...
app.post('/', async (c) => {
  ...
  const blocked = requireExistingDramaAccess(c, Number(body.drama_id), ['owner', 'producer', 'editor'])
...
app.put('/:id', async (c) => {
  ...
  const blocked = requireExistingDramaAccess(c, getDramaIdBySceneId(id), ['owner', 'producer', 'editor'])
  if (blocked) return blocked
  const body = await c.req.json()
...
app.post('/:id/generate-image', async (c) => {
  const id = Number(c.req.param('id'))
  const body = await c.req.json()
  const blocked = requireExistingDramaAccess(c, getDramaIdBySceneId(id), ['owner', 'producer'])
  if (blocked) return blocked
